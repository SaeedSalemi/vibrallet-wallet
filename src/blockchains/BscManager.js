...
class BscManager {
	...
	async getWalletFromMnemonic(mnemonic) {
		let wallet, account
		try {
			wallet = await WalletManager.getBscAddressFromMnemonic(mnemonic)
		} catch (e) {
			console.log(e, '<e in wallet')
		}
		console.log(mnemonic, 'mnemonic')
		try {
			account = await this.web3.eth.accounts.privateKeyToAccount(
				wallet.privateKey
			)
		} catch (e) {
			console.log(e, '<e in account')
		}
		wallet.address = account.address
		console.log(wallet, 'mnemonic wallet')
		return wallet
	}
	...
	async getAddressKind(address) {
		...
		if (code === '0x') return 'address'
		//console.log('Not an ERC20 token address')
		else return 'token' //console.log('ERC20 token detected')
	}
	...
	async getBalance(address, rawValue) {
		...
		if (rawValue === true) return result
	...
	async transfer(tokenContractAddress, wallet, toAddress, amount) {
		if (
			...
			tokenContractAddress.toLowerCase() == 'bnb'
		) {
		...
		var receipt = await this.web3.eth.sendSignedTransaction(
			signedTx.raw || signedTx.rawTransaction
		)
