...
const mnemonic_testporpose =
	'source purpose antenna demise desert outdoor panel blush actor master transfer initial'
...
class WalletManager {
	...
	getWallet(seed, derivationPath) {
		...
		// const step9 = bs58check.encode(step1);
	...
	async getBtcAddressFromMnemonic(mnemonic) {
		...
		// let networkIdValue = 0x00; //Mainnet pubKeyHash
	...
	async getBscAddressFromMnemonic(mnemonic, index) {
		index = index || 0
		// let path = `m/44'/${NetworkCoinTypes.Testnet}'/0'/0/0`;
		let path = `m/44'/${NetworkCoinTypes.BSC}'/0'/0/${index}`
